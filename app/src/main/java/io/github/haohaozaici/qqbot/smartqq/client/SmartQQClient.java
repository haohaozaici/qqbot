...
import io.github.haohaozaici.qqbot.smartqq.callback.MessageCallback;
import io.github.haohaozaici.qqbot.smartqq.constant.ApiURL;
import io.github.haohaozaici.qqbot.smartqq.model.UserInfo;
...
public class SmartQQClient {
  ...
  //日志
  private static final String TAG = "SmartQQClient";
...
//  private Session session;
  ...
  private void getQRCode() {
    ...
    Request request = new Request.Builder()
        .url(ApiURL.GET_QR_CODE.getUrl())
        .addHeader("User-Agent", ApiURL.USER_AGENT)
        .build();

    Call call = mNetwork.okHttpClient.newCall(request);
      ...
      public void onResponse(Call call, Response response) throws IOException {
        ...
        if (!StringUtils.isEmpty(cookies)) {
          ...
          for (String cookie : strings) {
            if (StringUtils.equals(cookie, "qrsig")) {
    ...
    XLog.i("二维码已保存在 " + " 文件中，请打开手机QQ并扫描二维码");
  }
  ...
  private String verifyQRCode() {
    ...
    while (true) {
      sleep(1);
      ResponseBody response = get(ApiURL.VERIFY_QR_CODE, hash33(qrsig));
  ...
  public void sendMessageToGroup(long groupId, String msg) {
    ...
    r.put("content", JSON.toJSONString(
  ...
  public List<Discuss> getDiscussList() {
    ...
    return JSON.parseArray(getJsonObjectResult(response).getJSONArray("dnamelist").toJSONString(),
        Discuss.class);
  }
